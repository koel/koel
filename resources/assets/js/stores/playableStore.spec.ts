...
describe('playableStore', () => {
  ...
  it('matches a song by exact title', () => {
  ...
  it('matches a song case-insensitively', () => {
    const song = h.factory('song', { title: 'An amazing song' })
    const songs = [song, ...h.factory('song', 3)]
  ...
  it('matches a song ignoring diacritics', () => {
    const song = h.factory('song', { title: 'Xin chào Việt Nam' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Xin chao Viet Nam', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('xin chào việt nam', songs)).toEqual(song)
  })

  it('matches a song with accented characters in search', () => {
    const song = h.factory('song', { title: 'Xin chao Viet Nam' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Xin chào Việt Nam', songs)).toEqual(song)
  })

  it('matches a song with Chinese characters', () => {
    const song = h.factory('song', { title: '你好世界' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('你好世界', songs)).toEqual(song)
  })

  it('matches a song with Japanese characters', () => {
    const song = h.factory('song', { title: 'こんにちは世界' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('こんにちは世界', songs)).toEqual(song)
  })

  it('matches a song with Korean characters', () => {
    const song = h.factory('song', { title: '안녕하세요 세계' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('안녕하세요 세계', songs)).toEqual(song)
  })

  it('matches a song with Arabic characters', () => {
    const song = h.factory('song', { title: 'مرحبا بالعالم' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('مرحبا بالعالم', songs)).toEqual(song)
  })

  it('matches a song ignoring punctuation and symbols', () => {
    const song = h.factory('song', { title: 'Hello, World!' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Hello World', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('Hello, World!', songs)).toEqual(song)
  })

  it('matches a song ignoring hyphens and dashes', () => {
    const song = h.factory('song', { title: 'Fade - To Black' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Fade To Black', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('Fade - To Black', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('Fade — To Black', songs)).toEqual(song) // em dash
  })

  it('matches a song ignoring brackets and parentheses', () => {
    const song = h.factory('song', { title: 'Song Title (Remastered)' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Song Title Remastered', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('Song Title (Remastered)', songs)).toEqual(song)
  })

  it('matches a song ignoring square brackets', () => {
    const song = h.factory('song', { title: 'Song Title [Live]' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Song Title Live', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('Song Title [Live]', songs)).toEqual(song)
  })

  it('matches a song ignoring periods', () => {
    const song = h.factory('song', { title: 'Mr. Brightside' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Mr Brightside', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('Mr. Brightside', songs)).toEqual(song)
  })

  it('matches a song ignoring quotes', () => {
    const song = h.factory('song', { title: "Don't Stop Me Now" })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Dont Stop Me Now', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle("Don't Stop Me Now", songs)).toEqual(song)
  })

  it('matches a song ignoring extra whitespace', () => {
    const song = h.factory('song', { title: 'Hello   World' })
    const songs = [song, ...h.factory('song', 3)]
  ...
  it('matches European accented characters', () => {
    const song = h.factory('song', { title: 'Über den Wölken' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Uber den Wolken', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('über den wölken', songs)).toEqual(song)
  })

  it('matches a song with mixed scripts and diacritics', () => {
    const song = h.factory('song', { title: 'Café 你好' })
    const songs = [song, ...h.factory('song', 3)]

    expect(playableStore.matchSongsByTitle('Cafe 你好', songs)).toEqual(song)
    expect(playableStore.matchSongsByTitle('café 你好', songs)).toEqual(song)
  })
