...
new class extends UnitTestCase {
  ...
  protected test () {
    it('gets a song by ID', () => {
      ...
      playableStore.vault.set('bar', reactive(factory('song', { id: 'bar' })))
    ...
    it('gets songs by IDs', () => {
      ...
      playableStore.vault.set('baz', reactive(factory('song', { id: 'baz' })))

      expect(playableStore.byIds(['foo', 'bar'])).toEqual([foo, bar])
    })

    it('gets formatted length', () => {
      expect(playableStore.getFormattedLength(factory('song', { length: 123 }))).toBe('2 min 3 sec')
    ...
    it('updates songs', async () => {
      ...
      const syncSongsMock = this.mock(playableStore, 'syncWithVault')
    ...
    it('syncs with the vault', () => {
      ...
      const watchPlayCountMock = this.mock(playableStore, 'watchPlayCount')
    ...
    it('paginates', async () => {
      ...
      const syncMock = this.mock(playableStore, 'syncWithVault', reactive(songs))
    ...
    it('toggles favorite to true', async () => {
      playableStore.state.favorites = factory('song', 2, { favorite: true })
    ...
    it('toggles favorite to false', async () => {
      playableStore.state.favorites = factory('song', 3, { favorite: true })
