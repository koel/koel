...
import Component from './ArtistTableHeaderActionMenu.vue'

describe('artistTableHeaderActionMenu.vue', () => {
  ...
  it('renders all toggleable columns as menu rows', async () => {
    ...
    for (const label of ['Name', 'Rating', 'Favorite']) {
  ...
  it('emits sort when a menu row is clicked', async () => {
    ...
    expect(emitted('sort')?.[0]).toEqual(['favorite'])
  })
