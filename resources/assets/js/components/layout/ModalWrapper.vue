<template>
  <dialog
    ...
    @keydown.esc.prevent
  >
    <Component :is="modalNameToComponentMap[activeModalName]" v-if="activeModalName" v-bind="props" @close="close" />
  </dialog>
</template>
...
const modalNameToComponentMap: Record<keyof Modals, Closure> = {
  ...
  CREATE_SMART_PLAYLIST_FORM: defineAsyncComponent(
  ...
  EDIT_SMART_PLAYLIST_FORM: defineAsyncComponent(
...
const showModal = <N extends keyof Modals>(name: N, ...args: [Modals[N]] extends [never] ? [] : [ctx: Modals[N]]) => {
  props.value = (args[0] ?? {}) as Modals[N]

  props.value.toggleCssClass =
    'toggleCssClass' in props.value
      ? props.value.toggleCssClass
...
eventBus
  .on('MODAL_SHOW_ABOUT_KOEL', () => showModal('ABOUT_KOEL'))
  ...
  .on('MODAL_SHOW_EDIT_PLAYLIST_FORM', playlist =>
    showModal(playlist.is_smart ? 'EDIT_SMART_PLAYLIST_FORM' : 'EDIT_PLAYLIST_FORM', { playlist }),
  )
