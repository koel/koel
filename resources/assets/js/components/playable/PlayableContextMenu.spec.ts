...
new class extends UnitTestCase {
  ...
  protected test () {
    ...
    it('adds to favorites', async () => {
      const likeMock = this.mock(playableStore, 'favorite')
      const { playables } = await this.renderComponent()
    ...
    it('removes from favorites', async () => {
      const unlikeMock = this.mock(playableStore, 'undoFavorite')
    ...
    it('deletes song', async () => {
      ...
      const deleteMock = this.mock(playableStore, 'deleteSongsFromFilesystem')
      const { playables } = await this.beAdmin().renderComponent()
    ...
    it('makes songs private', async () => {
      ...
      const privatizeMock = this.mock(playableStore, 'privatizeSongs').mockResolvedValue(songs.map(song => song.id))
    ...
    it('makes songs public', async () => {
      ...
      await this.be(user).renderComponent(songs)
      const publicizeMock = this.mock(playableStore, 'publicizeSongs').mockResolvedValue(songs.map(song => song.id))
