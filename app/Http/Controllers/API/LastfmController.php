...
class LastfmController extends Controller
{
    ...
    {
        abort_unless($lastfm->enabled(), 401, trans('messages.last_fm_disabled'));
    ...
    {
        abort_unless($token = $request->input('token'), 500, trans('messages.something_wrong'));

        // Get the session key using the obtained token.
        abort_unless($sessionKey = $lastfm->getSessionKey($token), 500, trans('messages.invalid_token'));
